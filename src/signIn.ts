...
export class MainAppModal extends Modal {
    ...
    for (const team of teams) {
      ...
      for (const space of spaces) {
        ...
        for (const folder of folders || []) {
          ...
          fList.forEach((l: any) => lists.push({ name: l.name, id: l.id }));
        }
        const folderless = (await getFolderlessList(space.id)) || [];
        folderless.forEach((l: any) => lists.push({ name: l.name, id: l.id }));
      }
    }
    return lists;
  }
  ...
  renderAuthrization() {
    ...
    const clientIdInput = createInputWithPlaceholder(
      "Client ID",
      "Enter client ID",
      true
    );
    const secretInput = createInputWithPlaceholder(
      "Secret",
      "Enter secret",
      true
    );
    ...
    const enableButton = () => {
      const clientId = clientIdInput.element.value;
      const secret = secretInput.element.value;
      button.disabled = !(clientId && secret);
    };

    clientIdInput.element.addEventListener("input", enableButton);
    secretInput.element.addEventListener("input", enableButton);
    ...
    button.addEventListener("click", async () => {
      ...
      if (!inputValue) {
        window.open(
          "https://app.clickup.com/api?client_id=" +
            clientId +
            "&redirect_uri=http://localhost:4010/obsidian",
          "_blank"
        );
      }
      ...
      if (clientId && secret && inputValue) {
        button.disabled = true;
        button.textContent = "Loading...";
        button.classList.add("renderAuthrizationBtn");

        token = await getToken(inputValue, clientId, secret);
        if (token) {
          await plugin.fetchUser(token);
          button.classList.add("renderAuthrizationBtnSucces");
    ...
    steps.appendChild(clientIdInput.element);
    steps.appendChild(secretInput.element);
